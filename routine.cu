// This file includes the functions called by CPU, running on GPU.

#include "buffers.cu"
#include "subroutine.cu"

// initialization 
__global__ void Initialize( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double *ci, double *cinext, double *cnsr, double *cnsrnext, 
						...
						double sv_ica, double sv_ncx)
{
	...
	{	
		...
		double ICa = 0;
			...
			{
				jj = LTCC_gating( &localState, CRU2[ps].lcc[ll], (CRU2[ps].cp+cs[ps]*icagamma)/(1+icagamma), v);
				CRU2[ps].lcc[ll] = jj;
		...
		int problem = RyR_gating( &localState, Ku, Kb, cpstar, CRU2[ps].cp, CRU2[ps].cjsr, &CRU2[ps].ncu, &CRU2[ps].nou, 
		...
		for ( int ii = 0; ii < 8; ++ii )
		{
			...
			xicoupn[ii] =	(cnsr[north]-cnsr[psi])/(taunt*xi) +
							(cnsr[south]-cnsr[psi])/(taunt*xi) +
							(cnsr[east]-cnsr[psi])/(taunt*xi) +
							(cnsr[west]-cnsr[psi])/(taunt*xi) +
							(cnsr[top]-cnsr[psi])/(taunl*xi) +
							(cnsr[bottom]-cnsr[psi])/(taunl*xi) ;

			xicoupi[ii] =	(ci[north]-ci[psi])/(tauit*xi) +
							(ci[south]-ci[psi])/(tauit*xi) +
							(ci[east]-ci[psi])/(tauit*xi) +
							(ci[west]-ci[psi])/(tauit*xi) +
							(ci[top]-ci[psi])/(tauil*xi) +
							(ci[bottom]-ci[psi])/(tauil*xi) ;
